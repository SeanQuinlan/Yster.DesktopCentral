function Get-DCAPIComputerAvailableHardware {
    <#
    .SYNOPSIS
        Gets the list of available hardware on the specified system.
    .DESCRIPTION
        Returns a list of hardware categories that are available on the specified resource ID.
    .EXAMPLE
        Get-DCAPIComputerAvailableHardware -HostName DCSERVER -AuthToken '47A1157A-7AAC-4660-XXXX-34858F3A001C' -ResourceID 101

        Returns an object containing available hardware categories for the resource ID 101.
    .NOTES
    #>
    ...
    try {
        ...
        $API_Path = 'dcapi/inventory/computers/{0}/availableHardware' -f $ResourceID
        $Query_Parameters = @{
            'APIPath' = $API_Path
            'Method'  = 'GET'
        }
