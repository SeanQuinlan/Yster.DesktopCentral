function Invoke-DCAPIComputerScan {
    <#
    .SYNOPSIS
        Start the inventory (agent) scan process on the supplied resource.
    .DESCRIPTION
    ...
    .EXAMPLE
        Invoke-DCAPIComputerScan -HostName DCSERVER -AuthToken '47A1157A-7AAC-4660-XXXX-34858F3A001C' -ResourceName SRV1

        Starts the inventory scan process on the resource named "SRV1".
    .NOTES
    #>
    ...
    try {
        ...
        [void]$ShouldProcess_Statement.AppendLine(('Start an inventory scan on this computer: {0}' -f $ResourceID))
